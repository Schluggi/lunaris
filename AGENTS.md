...
| [`src/frozen_rx.rs`](src/frozen_rx.rs) | Parses inbound Frozen payloads: `0x81` pong, `0x90` jump ack (wake state); **`0x41` TemperatureUpdate**, **`0xC1` GetTemperature** → MQTT current temps + heatsink (opensleep `frozen/packet.rs`); **`0x07` Message** — `FW: water full → empty` / `FW: water empty → full` (opensleep `frozen/state.rs`) → optional MQTT **Water Tank** binary_sensor. |
| [`src/frozen_link.rs`](src/frozen_link.rs) | **Opens Frozen UART once** — opensleep-style **wake** (`Ping` + `JumpToFirmware` every 2s until pong shows firmware), **keepalive** ping ~15s, queues MQTT frames (`mpsc`); forwards decoded temperatures and optional water-tank events to **`mqtt_bridge`**. |
...
| [`src/sensor_rx.rs`](src/sensor_rx.rs) | Parses inbound **`0xAE` VibrationEnabled** (and related acks); required before `SetAlarm` works like opensleep’s state machine. Parses **`0x33` Capacitance** (opensleep `sensor/packet.rs`) → MQTT **occupancy** left/right (`max` of zones 0–2 / 3–5 vs threshold). |
...
| [`src/sensor_link.rs`](src/sensor_link.rs) | **Opens Sensor UART once** — RX decode, periodic **piezo priming**, optional forward of capacitance to MQTT, queues vibration: **priming → wait for `0xAE` → SetAlarm**. MQTT **Priming** binary_sensor mirrors [`PrimingCounts`] (opensleep has no equivalent HA entity). Piezo setup follows opensleep-style staggering (`CONFIG_RES_TIME`–style gaps in [`sensor_link`](src/sensor_link.rs)); upstream schedules individual commands instead of bulk periodic batches. |
...
| [`src/mqtt_bridge.rs`](src/mqtt_bridge.rs) | **rumqttc**: discovery Prime + optional **Vibrate** + optional **Priming** binary_sensor + optional **`device_class`: occupancy** **Presence Left / Right** (`…/binary_sensor/presence_left|right/state`, from Sensor capacitance **`0x33`**) + optional **Water Tank** binary_sensor (`device_class`: **plug**; Frozen `0x07`; **ON** = reservoir present, **OFF** = removed; diagnostic) + **three Frozen temperature sensors** (HA names **Current Temperature Left/Right**, heatsink °C; state topics still `cover_temperature_*`) + **two target-temperature sensors** (HA **Target Temperature Left/Right**; state `…/sensor/target_temperature_left|right/state`, mirrors climate setpoint) + **climate left/right** (MQTT **`current_temperature_topic`** = same as each side’s `cover_temperature_*` state, see [climate.mqtt](https://www.home-assistant.io/integrations/climate.mqtt/)) + optional light + optional **Startup LED** switch (retained preference; green LED only when narcolepsy **starts** if ON); enqueues bytes to **`frozen_tx` / `sensor_tx`**; outbound in **`tokio::spawn`** (avoid deadlock with `poll()`). |

Typical MQTT topics: `…/button/prime/set`, `…/button/vibrate_left|vibrate_right/set`, `…/binary_sensor/priming/state`, `…/binary_sensor/presence_left|right/state`, `…/binary_sensor/water_tank/state`, `…/sensor/cover_temperature_left|right/state`, `…/sensor/heatsink_temp/state`, `…/sensor/target_temperature_left|right/state`, `…/climate/…`, `…/switch/startup_led/set|state`, `…/result`.
...
**Frozen serial:** default **`/dev/ttyS1`** @ 38400 (Pod 4). **`--no-water-tank-sensor`** disables MQTT **Water Tank** (no `0x07` forwarding). **Sensor (vibration):** default **`/dev/ttyS2`** @ **`38400`** (Pod 4 RX framing; use **`115200`** for opensleep Pod 3–style firmware) — `--no-vibration` or open failure → no vibration buttons **and** no capacitance presence. When the Sensor UART is enabled, MQTT discovers **presence** (**`--presence-cap-threshold`**, default **800**: each side compares the **maximum** of its three zones to this raw u16) unless **`--no-presence-detection`**. On startup, **`sensor_link`** runs an opensleep-style **bootloader handshake** (38400 Ping + JumpToFirmware, then opens firmware baud) unless **`--no-sensor-bootloader-handshake`**. **`--sensor-vibrate-no-ack-wait`** skips waiting for inbound **`0xAE`** (for Pods where RX is not opensleep-framed at the chosen baud). Each vibrate sends EnableVibration + piezo setup + SetAlarm (matches opensleep sensor manager ordering).
