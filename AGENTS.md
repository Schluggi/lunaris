...
| [`src/deviceinfo.rs`](src/deviceinfo.rs) | Reads **`/deviceinfo/device-label`** and **`/deviceinfo/device-id`** on the host; MQTT text sensors publish the same paths as **retained** state topics (empty payload if a file is missing); HA discovery sets **`enabled_by_default`: false** (like **Firmware message**). |
...
Typical MQTT topics: `…/button/prime/set`, `…/button/request_get_temperatures/set`, `…/button/vibrate_left|vibrate_right/set`, `…/button/calibrate_presence/set`, `…/binary_sensor/presence_left|right|any/state`, `…/binary_sensor/water_tank/state`, `…/sensor/firmware_message/state`, `…/sensor/cover_temperature_left|right/state`, `…/sensor/heatsink_temp/state`, `…/sensor/target_temperature_left|right/state`, `…/climate/…`, `…/switch/startup_led/set|state`, `…/result`, plus **retained** diagnostic text sensors **`/deviceinfo/device-label`** and **`/deviceinfo/device-id`** (payloads read from the same host paths under `/deviceinfo/` when present).
