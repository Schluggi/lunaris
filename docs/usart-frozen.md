...
| SetTargetTemperature | `40` then `side`, `en`, `T_hi`, `T_lo` bytes | `7E 05 40 00 01 0E 10 E6 A8` (example) | Side `00` = left, `01` = right; `en` `01` = enabled; target `T` is u16 BE in centidegrees °C (`0x0E10` → 36.00 °C). Opensleep `FrozenCommand::SetTargetTemperature`. |
