...
- Process runs **read/write as the same user** that owns `narcolepsy`; you may need root or membership in the right group if udev restricts `/dev/i2c-*`.
...
- Use **`--no-led`** to skip I²C entirely (no probe, no light discovery).
